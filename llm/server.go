...
type LoadRequest struct {
	Commit bool // Load models weights after allocation
	Close  bool // Free existing memory and return - other fields are ignored
			...
			for j := range memory.GPUs {
				if gl[i].ID == memory.GPUs[j].ID {
					...
					slog.Debug("available gpu", "id", gl[i].ID,
						...
						"backoff", backoff, "minimum", format.HumanBytes2(gl[i].MinimumMemory),
